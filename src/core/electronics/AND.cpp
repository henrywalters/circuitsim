...
void ANDGate::create() {
    ...
    gate->texture.path("textures/circuits/AND");
...
void ANDGate::evaluate(CircuitRunner* circuit, std::vector<PinState> const &inputs, std::vector<PinState> &outputs) {
    outputs[0] = inputs[0] == PinState::HIGH && inputs[1] == PinState::HIGH ? PinState::HIGH : PinState::LOW;
}
